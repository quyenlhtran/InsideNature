...
const sun=new T.DirectionalLight(0xffe8ac,3.2);sun.position.set(-18,28,12);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);sun.shadow.camera.left=-35;sun.shadow.camera.right=35;sun.shadow.camera.top=35;sun.shadow.camera.bottom=-35;scene.add(sun);
...
const sphere=(r:number,color:T.ColorRepresentation)=>mesh(new T.SphereGeometry(r,18,12),mat(color));
const box=(x:number,y:number,z:number,color:T.ColorRepresentation)=>mesh(new T.BoxGeometry(x,y,z),mat(color));
const cylinder=(top:number,bottom:number,height:number,color:T.ColorRepresentation,segments=10)=>mesh(new T.CylinderGeometry(top,bottom,height,segments),mat(color));
let seed=481516;const random=()=>{seed=(seed*16807)%2147483647;return(seed-1)/2147483646};

// Forest floor and winding river, based on the visual layers of the supplied illustration.
const leftLand=box(30.5,1.2,66,'#527849'),rightLand=box(26.5,1.2,66,'#527849');leftLand.position.set(-19.75,-.65,0);rightLand.position.set(21.75,-.65,0);scene.add(leftLand,rightLand);
const riverMaterial=new T.MeshPhysicalMaterial({color:'#45baca',transparent:true,opacity:.78,roughness:.18,metalness:.05,transmission:.12,side:T.DoubleSide});
const river=new T.Mesh(new T.PlaneGeometry(13,66,12,22),riverMaterial);river.rotation.x=-Math.PI/2;river.rotation.z=-.08;river.position.set(2,.025,0);scene.add(river);
const bankMat=mat('#ad8d55');for(const side of[-1,1]){const bank=box(.7,.25,65,'#b08a50');bank.position.set(2+side*6.5,.03,0);bank.rotation.y=.08;scene.add(bank)}
const bridge=new T.Group();for(let i=-4;i<=4;i++){const plank=box(1.1,.16,2.8,'#9a6338');plank.position.set(i*1.02,.55,2);plank.rotation.z=Math.sin(i*.45)*.045;bridge.add(plank)}for(const z of[.85,3.15]){const rail=box(10,.12,.12,'#774729');rail.position.set(0,1.18,z);bridge.add(rail)}bridge.position.set(2,0,0);scene.add(bridge);

function tree(x:number,z:number,scale:number){const g=new T.Group();const trunk=cylinder(.28,.42,3.7,'#66452d',9);trunk.position.y=1.8;g.add(trunk);for(const [y,r,c] of [[3.2,1.9,'#2f6741'],[4.25,1.55,'#3f7b49'],[5.15,1.05,'#4c8b50']] as [number,number,string][]){const crown=mesh(new T.ConeGeometry(r,2.7,10),mat(c));crown.position.y=y;g.add(crown)}g.position.set(x,0,z);g.scale.setScalar(scale);scene.add(g);return g}
for(let i=0;i<42;i++){let x=(random()-.5)*58,z=(random()-.5)*58;if(Math.abs(x-2)<8)x+=x<2?-8:8;tree(x,z,.72+random()*.65)}
for(let i=0;i<90;i++){const blade=mesh(new T.ConeGeometry(.08,.6+random()*.5,5),mat(random()>.4?'#6f984e':'#92aa50'),false);blade.position.set((random()-.5)*56,.3,(random()-.5)*56);scene.add(blade)}
for(let i=0;i<28;i++){const flower=new T.Group();const stem=cylinder(.025,.025,.5,'#48733d',5);stem.position.y=.25;flower.add(stem);for(let p=0;p<5;p++){const petal=sphere(.09,random()>.5?'#f5d7dc':'#f4dd73');petal.scale.set(1.6,.55,1);petal.position.set(Math.cos(p*1.256)*.12,.53,Math.sin(p*1.256)*.12);flower.add(petal)}flower.position.set((random()-.5)*46,0,(random()-.5)*46);scene.add(flower)}

// Underwater world.
const seabed=mesh(new T.CylinderGeometry(34,35,1,48),mat('#3d7770'),false);seabed.position.y=-11.5;scene.add(seabed);
for(let i=0;i<45;i++){const rock=sphere(.25+random()*.7,random()>.5?'#517a74':'#8c8c72');rock.scale.set(1.3,.55,1);rock.position.set((random()-.5)*48,-10.7,(random()-.5)*48);scene.add(rock)}
for(let i=0;i<35;i++){const weed=mesh(new T.ConeGeometry(.09+random()*.08,1+random()*2,7),mat(random()>.5?'#2d8771':'#6aa764'),false);weed.position.set((random()-.5)*45,-10+random()*.4,(random()-.5)*45);weed.rotation.z=(random()-.5)*.25;scene.add(weed)}
const bubbles:T.Mesh[]=[];for(let i=0;i<42;i++){const bubble=new T.Mesh(new T.SphereGeometry(.04+random()*.08,8,6),new T.MeshBasicMaterial({color:'#c8f5ed',transparent:true,opacity:.5,wireframe:true}));bubble.position.set((random()-.5)*45,-10+random()*9,(random()-.5)*45);bubble.userData.speed=.25+random()*.5;bubbles.push(bubble);scene.add(bubble)}

// Sky landmarks.
const cloudMaterial=new T.MeshStandardMaterial({color:'#fff7df',roughness:1,transparent:true,opacity:.87});
for(let i=0;i<13;i++){const cloud=new T.Group();for(let j=0;j<5;j++){const puff=new T.Mesh(new T.SphereGeometry(1.2+random(),12,8),cloudMaterial);puff.scale.y=.55;puff.position.set(j*1.35+(random()-.5),random()*.6,(random()-.5)*1.4);cloud.add(puff)}cloud.position.set((random()-.5)*58,13+random()*13,(random()-.5)*58);scene.add(cloud)}

function animalBody(color:string,accent:string){const g=new T.Group();const body=sphere(.65,color);body.scale.set(1.35,.78,.72);body.rotation.y=Math.PI/2;g.add(body);const head=sphere(.43,color);head.position.set(0,.37,-.78);g.add(head);const eye=sphere(.055,'#101b17');eye.position.set(.3,.48,-1.12);g.add(eye);const eye2=eye.clone();eye2.position.x=-.3;g.add(eye2);const nose=sphere(.1,accent);nose.position.set(0,.28,-1.18);g.add(nose);return g}
function deer(){const g=animalBody('#a86a35','#33251d');g.scale.setScalar(.85);for(const x of[-.4,.4])for(const z of[-.3,.35]){const leg=cylinder(.07,.09,1.2,'#74472d',7);leg.position.set(x,-.72,z);g.add(leg)}for(const x of[-.23,.23]){const antler=cylinder(.035,.055,.8,'#d2b27b',6);antler.position.set(x,1,-.85);antler.rotation.z=x*1.5;g.add(antler)}return g}
function fox(){const g=animalBody('#c55e32','#342018');for(const x of[-.42,.42]){const ear=mesh(new T.ConeGeometry(.2,.55,5),mat('#9e432b'));ear.position.set(x,.9,-.77);g.add(ear)}const tail=mesh(new T.ConeGeometry(.35,1.8,9),mat('#d87542'));tail.rotation.x=-Math.PI/2;tail.position.set(0,.15,1.25);g.add(tail);return g}
function rabbit(){const g=animalBody('#9b8f7d','#594b42');g.scale.setScalar(.65);for(const x of[-.2,.2]){const ear=sphere(.18,'#a79c8c');ear.scale.set(1,2.8,.6);ear.position.set(x,1,-.72);g.add(ear)}return g}
function fish(color:string){const g=new T.Group();const body=sphere(.62,color);body.scale.set(1.55,.72,.45);g.add(body);const tail=mesh(new T.ConeGeometry(.55,1,3),mat(color));tail.rotation.z=-Math.PI/2;tail.position.x=-1.15;g.add(tail);const eye=sphere(.07,'#101b17');eye.position.set(.68,.2,.35);g.add(eye);return g}
function turtle(){const g=new T.Group();const shell=sphere(.7,'#527945');shell.scale.set(1.25,.45,1);g.add(shell);const head=sphere(.3,'#7aa068');head.position.x=1;g.add(head);for(const z of[-.65,.65])for(const x of[-.45,.45]){const fin=mesh(new T.ConeGeometry(.18,.65,5),mat('#71935f'));fin.rotation.z=Math.PI/2;fin.position.set(x,0,z);g.add(fin)}return g}
function ray(){const g=new T.Group();const wing=mesh(new T.SphereGeometry(.9,18,8),mat('#487b84'));wing.scale.set(1.7,.16,1);g.add(wing);const tail=cylinder(.03,.07,2.3,'#365e65',7);tail.rotation.z=Math.PI/2;tail.position.x=-1.6;g.add(tail);return g}
function bird(color:string){const g=new T.Group();const body=sphere(.35,color);body.scale.set(.75,.65,1.45);g.add(body);for(const side of[-1,1]){const wing=mesh(new T.ConeGeometry(.55,1.8,3),mat(color));wing.rotation.z=side*Math.PI/2;wing.position.x=side*.85;g.add(wing)}const beak=mesh(new T.ConeGeometry(.12,.5,5),mat('#e5b047'));beak.rotation.x=Math.PI/2;beak.position.z=-.7;g.add(beak);return g}
function butterfly(){const g=new T.Group();const body=cylinder(.06,.08,.65,'#2c211b',7);body.rotation.x=Math.PI/2;g.add(body);for(const side of[-1,1]){const wing=sphere(.38,side<0?'#e5903c':'#efb24c');wing.scale.set(.22,1,1.2);wing.position.x=side*.27;g.add(wing)}return g}
function tiger(){const g=animalBody('#d97827','#26180f');g.scale.setScalar(1.18);for(const x of[-.5,.5])for(const z of[-.3,.35]){const leg=cylinder(.1,.13,1.25,'#d97827',7);leg.position.set(x,-.78,z);g.add(leg)}for(let i=-2;i<=2;i++){const stripe=box(.07,.48,.72,'#2b2019');stripe.position.set(i*.25,.28,.05);stripe.rotation.z=i*.1;g.add(stripe)}for(const x of[-.27,.27]){const ear=mesh(new T.ConeGeometry(.2,.45,5),mat('#2b2019'));ear.position.set(x,.88,-.82);g.add(ear)}return g}
function frog(){const g=new T.Group();const body=sphere(.42,'#5d9b48');body.scale.set(1,.65,1.15);g.add(body);for(const x of[-.28,.28]){const eye=sphere(.14,'#9bc66c');eye.position.set(x,.34,-.25);g.add(eye);const pupil=sphere(.05,'#172418');pupil.position.set(x,.38,-.37);g.add(pupil)}for(const x of[-.5,.5]){const leg=box(.55,.12,.16,'#4f873e');leg.position.set(x,-.18,.22);g.add(leg)}return g}
function owl(){const g=new T.Group();const body=sphere(.55,'#8b7257');body.scale.set(.8,1.2,.72);g.add(body);for(const x of[-.2,.2]){const eye=sphere(.15,'#e6c35a');eye.position.set(x,.2,-.48);g.add(eye);const pupil=sphere(.06,'#16130f');pupil.position.set(x,.2,-.61);g.add(pupil)}const beak=mesh(new T.ConeGeometry(.1,.35,4),mat('#c9933c'));beak.rotation.x=Math.PI/2;beak.position.set(0,.02,-.65);g.add(beak);return g}
function otter(){const g=animalBody('#76513b','#241a14');g.scale.set(.9,.75,1.15);const tail=mesh(new T.ConeGeometry(.22,1.7,8),mat('#654331'));tail.rotation.x=-Math.PI/2;tail.position.set(0,-.05,1.3);g.add(tail);return g}

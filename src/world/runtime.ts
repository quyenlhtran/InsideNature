...
document.querySelector<HTMLDivElement>('#app')!.innerHTML=`
  ...
  <section class="generated-hud" id="generated-hud" hidden><div class="eyebrow" id="generated-engine">Created from your image</div><h2 id="generated-title"></h2><p id="generated-summary"></p><button id="new-scene" type="button">Create another scene</button></section>
...
document.querySelector('#progress-count')!.textContent=`0 / ${creatures.length}`;
 const found=new Set<number>();const keys=new Set<string>();let yaw=0,pitch=-.06,currentBiome:Biome='woodland',nearby=-1,dialogueOpen=false,helpOpen=false,generatedMode=false,dialogueSession=0,modeSession=0,started=false,musicOn=true,listenName='';
...
function activateGeneratedScene(result:Awaited<ReturnType<typeof generateScene>>){
 ...
 document.querySelector('#generated-engine')!.textContent=result.pipeline?`Planned by Nemotron · ${result.pipeline.renderStrategy}`:'Created from your image';
 ...
 document.querySelector('#ai-status')!.textContent=result.source==='nvidia'?'Nemotron scene ready':'Starter scene generated';document.querySelector('#intro')!.classList.add('hidden');
}
...
function restoreExampleWorld(){generatedScene?.dispose();generatedScene=null;generatedMode=false;exampleSceneObjects.forEach(object=>{object.visible=true});document.body.classList.remove('generated-mode');document.querySelector<HTMLElement>('#generated-hud')!.hidden=true;document.querySelector<HTMLElement>('#generated-info')!.hidden=true;scene.background=new T.Color(zones.woodland.color);scene.fog=new T.FogExp2(zones.woodland.fog,.018);camera.position.set(0,3.5,15);yaw=0;pitch=-.06;currentBiome='sky';setBiome('woodland')}
...
document.querySelector<HTMLFormElement>('#scene-creator')!.addEventListener('submit',async event=>{
 ...
 const requestSession=modeSession,button=document.querySelector<HTMLButtonElement>('#generate-scene')!;button.disabled=true;creatorError.textContent='';mixer.startMusic();showLoading('Nemotron is reading your picture','Building your 3D world','Nemotron is choosing a scene strategy and planning the important shapes, labels, and learning moments. Detailed pictures may take longer while every pipeline stage finishes.');
 try{const result=await generateScene(file);if(requestSession===modeSession)activateGeneratedScene(result)}catch(error){if(requestSession===modeSession){showStage(creatorStage);creatorError.textContent=error instanceof Error?error.message:'We could not build that scene. Please try another image.'}}finally{button.disabled=false}
});
document.querySelector<HTMLFormElement>('#profile')!.addEventListener('submit',async event=>{
 ...
 started=true;document.querySelector<HTMLButtonElement>('#home-button')!.hidden=false;document.querySelector('#intro')!.classList.add('hidden');syncLookHint();
});

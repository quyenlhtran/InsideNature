...
function activateGeneratedScene(result:Awaited<ReturnType<typeof generateScene>>){
 ...
 const modelName=result.source==='gemini'?'Gemini Pro':'NVIDIA Nemotron';document.querySelector('#generated-engine')!.textContent=result.pipeline?`${modelName} · ${result.pipeline.renderStrategy}`:'Created from your image';
...
document.querySelector<HTMLFormElement>('#scene-creator')!.addEventListener('submit',async event=>{
 ...
 const submitter=(event as SubmitEvent).submitter as HTMLButtonElement|null,provider=(submitter?.value==='gemini'?'gemini':'nvidia') as SceneProvider,modelName=provider==='gemini'?'Gemini Pro':'NVIDIA Nemotron';

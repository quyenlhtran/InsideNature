...
document.querySelector<HTMLDivElement>('#app')!.innerHTML=`
  ...
  <section class="help" id="help" role="dialog" aria-modal="true" aria-labelledby="help-title" hidden><article class="help-card"><button class="help-close" id="help-close" type="button" aria-label="Close how to play">×</button><div class="eyebrow">Explorer guide</div><h2 id="help-title">How to play</h2><div class="help-steps"><div><b>01</b><span><strong>Explore the world</strong>Use <kbd>W</kbd> <kbd>A</kbd> <kbd>S</kbd> <kbd>D</kbd> to move and the mouse to look around.</span></div><div><b>02</b><span><strong>Change your height</strong>Hold <kbd>Space</kbd> to rise and <kbd>Shift</kbd> to descend between forest, sky, and river.</span></div><div><b>03</b><span><strong>Discover life</strong>Approach an animal or plant, then press <kbd>E</kbd> or click it to open its field guide.</span></div><div><b>04</b><span><strong>Complete the journey</strong>Answer each question correctly and discover all 15 species to fill your field journal.</span></div></div><p>Tip: use the biome buttons on the right to travel quickly.</p></article></section>
...
document.addEventListener('keydown',event=>{if(event.code==='Escape'&&helpOpen){setHelpOpen(false);return}keys.add(event.code);if(event.code==='KeyE'&&nearby>=0&&!dialogueOpen&&!helpOpen)openDialogue(nearby);if(event.code==='Escape'&&dialogueOpen)closeDialogue()});document.addEventListener('keyup',event=>keys.delete(event.code));

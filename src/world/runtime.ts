...
document.querySelector<HTMLFormElement>('#scene-creator')!.addEventListener('submit',async event=>{
 ...
 const button=document.querySelector<HTMLButtonElement>('#generate-scene')!;button.disabled=true;creatorError.textContent='';mixer.startMusic();showLoading('Nemotron is reading your picture','Building your 3D world','Nemotron is choosing a scene strategy and planning the important shapes, labels, and learning moments. Detailed pictures may take longer while every pipeline stage finishes.');
...
document.querySelector<HTMLFormElement>('#profile')!.addEventListener('submit',async event=>{
 ...
 started=true;document.querySelector('#intro')!.classList.add('hidden');syncLookHint();
});
